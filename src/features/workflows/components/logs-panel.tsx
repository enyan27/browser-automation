...
function ReplayRow({
  ...
  return (
    <button
      ...
      onClick={() => (isLocked ? goToUpgrade() : onSelect({ kind: "replay", runId: run.id }))}
    ...
    >
      ...
      {isLocked && <Lock className="ml-auto size-3.5 shrink-0 text-muted-foreground" />}
    </button>
  )
}
      ...
      {runs.map(run => (
        <div key={run.id} className="flex flex-col gap-0.5">
          ...
          {run.browserbaseSessionId && !run.isLive && (
            <ReplayRow
              run={run}
              isSelected={selected?.kind === "replay" && selected.runId === run.id}
              onSelect={onSelect}
            />
          )}
        </div>
