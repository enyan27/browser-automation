...
export function WorkflowNav({ workflows, onCreateWorkflow }: WorkflowNavProps) {
  ...
  const workflowItems = workflows.map(workflow => (
    <SidebarMenuItem key={workflow.id}>
      <SidebarMenuButton asChild isActive={pathname === `/workflows/${workflow.id}`}>
            ...
            <SidebarMenuItem>
              <Popover>
                ...
                <PopoverContent side="right" align="start" className="p-1">
                  <SidebarMenu>
                    <SidebarMenuItem>
                      <SidebarMenuButton onClick={handleCreateWorkflow} disabled={isPending}>
                        <PlusIcon />
  ...
  return (
    <SidebarGroup>
      ...
      <SidebarGroupAction title="New workflow" onClick={handleCreateWorkflow} disabled={isPending}>
        <PlusIcon />
